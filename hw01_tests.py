'''
Spring 2022 Homework 1 Test Suite

Handmade testing suite to be used with a colab notebook.
Originally made for Fall 2020 version of HW1.

To use:
1. Make sure this file is in the homework/hw01/_STATIC_/ folder

2. In Colab notebook, have a cell:

    !rm hw01_tests.py
    !wget --no-check-certificate https://introml.odl.mit.edu/cat-soop/_static/6.036/homework/hw01/hw01_tests.py
    from hw01_tests.py import *

3. Under each function to test, create a new cell with the function corresponding
   to the function you want to test, and pass in the student's implementation
   as an argument to that function. For example, to test the student's rv() function:
...
def TestCreateArray(arr):
    ...
    try:
        ...
        print('FAILED: Either your input is not an numpy array or its shape is incorrect.')

def TestTranspose(fn):
    '''
    Test function for transpose `tp` function
    '''
    cases = [(np.array([[1,2,3],[4,5,6]]), np.array([[1, 4], [2, 5], [3, 6]])),
...
def TestRowVector(fn):
    ...
    '''
    cases = [ ( [1,2], np.array([[1,2]]) ),
              ( [1,3,5], np.array([[1,3,5]])) ]
    RunTestSuite(cases, fn)
...
def TestLength(fn):
    ...
    if np.shape(output) != np.shape(1.0):
      print('FAILED: The output from your function to compute length must be a scalar.')
      return
    cases = [(np.array([[1],[2]]), 2.23606797749979),
...
def TestNormalize(fn):
    ...
    '''
    cases = [(np.array([[1],[2]]), np.array([[0.4472135954999579], [0.8944271909999159]]) ),
             (np.array([[1],[3],[5]]), np.array([[0.1690308509457033], [0.50709255283711], [0.8451542547285166]]) )]
    RunTestSuite(cases, fn)
...
def TestIndexing(fn):
    ...
    '''
    cases = [(np.array([[1,2],[3,4]]), np.array([[2], [4]]) ),
...
def TestInverse(fn):
    ...
    '''
    cases = [( np.array([[1,0],[0,1]]), np.array([[1,0],[0,1]]) ),
             ( np.array([[1,2],[3,4]]), np.array([[-2,1],[1.5,-0.5]]) ),
             ( np.array([[1,2,3],[3,4,5],[2,6,7]]), np.array([[-1/3,2/3,-1/3],[-11/6,1/6,2/3],[5/3,-1/3,-1/3]]) )]
    RunTestSuite(cases, fn)
...
def TestData(data):
    ...
    '''
    cases = [(data, [[150.0, 5.8], [130.0, 5.5], [120.0, 5.3]]) ]
...
def TestTransform(fn):
    ...
    '''
    cases = [(np.array([[150, 130, 120], [5.8, 5.5, 5.3]]), np.array([[155.8, 135.5, 125.3]]))]
    RunTestSuite(cases, fn)
...
def TestLinPredict(fn):
    ...
    cases = [
        ...
        ( np.array([[1.5],[-2]]), np.array([[2.0],[-3.0]]), np.array([[-2.0]]), np.array([[7.0]])  )
    ]
    RunTestSuite(cases, fn)
...
def TestLinPredict_N(fn):
    ...
    '''
    cases = [
        ( np.array([[1.5,3.],[-2.,1.]]), np.array([[2.],[-3.]]), np.array([[-2.]]), np.array([[7.0, 1.0]]) )
    ]
    RunTestSuite(cases, fn)
...
def TestMSE(fn):
    ...
    '''
    cases = [
        ( np.array([[1.5,3.]]), np.array([[2., 2.]]), np.array([[1.25/2]]) ),
        ( np.array([[-2.,1.,3.]]), np.array([[2.,1.,2.]]), np.array([[17/3]]) )
    ]
    RunTestSuite(cases, fn)
...
def TestMSE_N(fn):
    ...
    cases = [
        ( np.array([[1.5,3.]]), np.array([[2., 2.]]), np.array([[1.25/2]]) ),
        ( np.array([[1.5,3.,4],[-2.,1.,3.]]), np.array([[2., 2.,0.],[2.,1.,2.]]), np.array([[17.25/3], [17/3]]) )
    ]
    RunTestSuite(cases, fn)
...
def TestLinRegError(fn):
    ...
    '''
    cases = [
        ( np.array([[1.5,3.]]), np.array([[2.1,4.5]]), np.array([[2.]]), np.array([[-1.]]), np.array([[0.13]]) ),
        ( np.array([[1.5,3.],[-2.,1.]]), np.array([[1.,3.]]), np.array([[2.],[-3.]]), np.array([[-2.]]), np.array([[20.]]) )
    ]
    RunTestSuite(cases, fn)
...
def TestSignedDist(fn):
    cases = [(np.array([[0],[0]]), np.array([[3],[4]]), 5, np.array([[1.0]]) ),
...
def TestScore(fn):
    ...
    cases = [(data, labels, np.array([[1],[1]]), -2, 1),
...
def TestBestSeparator(fn):
    data = np.transpose(np.array([[1, 2], [1, 3], [2, 1], [1, -1], [2, -1]]))
    labels = np.array([[-1, -1, +1, +1, +1]])
    thetas = np.array([[ 0.98645534, -0.02061321, -0.30421124, -0.62960452,  0.61617711,
                         0.17344772, -0.21804797,  0.26093651,  0.47179699,  0.32548657],
                       [ 0.87953335,  0.39605039, -0.1105264 ,  0.71212565, -0.39195678,
                         0.00999743, -0.88220145, -0.73546501, -0.7769778 , -0.83807759]])
    theta_0s = np.array([[ 0.65043158],
                         [ 0.61626967],
                         [ 0.84632592],
                         [-0.43047804],
                         [-0.91768579],
